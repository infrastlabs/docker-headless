...
compile)
    ...
    # docker buildx build --platform linux/amd64,linux/arm64 --push -t $ns/$img -f src/Dockerfile.compile.t1 . 
    docker buildx build $plat --push -t $ns/$img -f src/Dockerfile.compile.arm . 
...
*)
    ...
    plat="--platform linux/amd64,linux/arm64"
    # docker build $cache $pull -t $repo/$ns/$img --build-arg FULL=/.. -f src/Dockerfile.arm . 
    # --build-arg FULL=/..
    docker  buildx build $plat --push -t $repo/$ns/$img -f src/Dockerfile.arm . 
    docker push $repo/$ns/$img    
    ;;
esac